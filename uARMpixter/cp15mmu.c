	...
	switch (CRn) {
		...
		case 1:		//control register
			...
			if (op2 == 0 && CRm == 0) {
				if (read) {
					...
					val = (cp15->control &~ 2) | (cpuAlignCheckingStateGet(cp15->cpu) ? 2 : 0);
				}
				else {
					...
					cp15->control ^= tmp;	//other bits
					cpuAlignCheckingStateSet(cp15->cpu, !!(val & 2));
				}
			}
			else if (CRm == 1) {
		...
		case 7:		//cache ops
			if ((CRm == 5 || CRm == 7) && op2 == 0) {
				//invalidate entire {icache(5) or both i and dcache(7)}
				if (CRm == 7) {
				...
				//invalidate {icache(5) or both i and dcache(7)} line, given VA
				if (CRm == 7) {
				...
				//invalidate {icache(5) or both i and dcache(7)} line, given set/index. i dont know how to do this, so flush the whole thing
				if (CRm == 7) {
