...
static bool ramAccessF(void* userData, uint32_t pa, uint_fast8_t size, bool write, void* bufP)
{
	...
	pa -= ram->adr;
	pa %= ram->sz;		//ram wraps around the address space it is in
...
struct ArmRam* ramInitEx(struct ArmMem *mem, uint32_t adr, uint32_t memSize, uint32_t addrSpaceSize, uint32_t* buf)
{
	...
	if (!memRegionAdd(mem, adr, addrSpaceSize, ramAccessF, ram))
...
{
	return ramInitEx(mem, adr, sz, sz, buf);
}
