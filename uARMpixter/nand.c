		...
		case K9nandStateReading:
			...
			if (nand->addrBytesRxed == 0xfe) {
				...
				if (nand->pageNo >= (nand->blocksPerDevice << nand->pagesPerBlockLg2)) {
					...
					fprintf(stderr, "page number ouf of bounds: %u in a device of %u blocks of %u pages\n", nand->pageNo, nand->blocksPerDevice, 1 << nand->pagesPerBlockLg2);
					return false;
				}
...
struct NAND* nandInit(const struct PixterRomChunk *chunk, const struct NandSpecs *specs, NandReadyCbk readyCbk, void *readyCbkData)
{
	...
	if (chunk)
		memcpy(nand->data, chunk->data, chunk->length < nandSz ? chunk->length : nandSz);
