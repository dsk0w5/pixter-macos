...
struct ArmCpu {
	...
	bool haveLongMultiplies, haveV5plus, haveV4plus, haveBx, haveMem64ops, haveExtraLoads, haveDspExts;
	bool haveThumb, baseUpdatedExcModel, doesAlignCheck, canConfigAlignCheck, hasPcBit1;

	//haveLongMultiplies	SMULL SMALAL UMULL UMLAL
	//haveMem64ops 			LDRD MCRR MRRC PLD STRD
	//haveV5plus			BKPT BLX CDP2 CLZ LDC2 MCR2 MRC2 STC2, loads to PC handle interworking
	//haveExtraLoads		LDRH LDRSB LDRSH STRH
	//haveDspExts			QADD QDADD QSUB QDSUB SMLAxy SMLALxy SMLAWy SMULxy SMULWy
	//haveBx				BX
	//haveThumb				as named
	//baseUpdatedExcModel	as named
	//haveV4plus			has system mode
};
...
struct ArmCP15* cpuGetCP15(struct ArmCpu *cpu)
{
	return cpu->cp15;
}
...
static void cpuPrvSetPCinterworked(struct ArmCpu *cpu, uint32_t pc)	//with interworking
{
	if ((pc & 3) == 2)
		fprintf(stderr, "Attempt to branch to non-word-aligned ARM address from around 0x%08x\n", cpu->regs[REG_NO_PC]);

	cpu->regs[REG_NO_PC] = pc &~ 1UL;
	cpu->T = (pc & 1);
}

static void cpuPrvSetPCuninterworked(struct ArmCpu *cpu, uint32_t pc)	//with no interworking
{
	uint32_t clrMask = cpu->T ? 1 : (cpu->hasPcBit1 ? 1 : 3);

	pc &=~ 1;	//always clear bottom bit;

	if (cpu->T && (pc & clrMask))
		fprintf(stderr, "V5 corner case. at pc 0x%08x, 0x%08x is written to PC, actually 0x%08x will be used\n", cpu->curInstrPC, pc, pc &~ clrMask);

	if (!cpu->T && (pc & clrMask))
		fprintf(stderr, "V5 corner case. at pc 0x%08x, 0x%08x is written to PC, actually 0x%08x will be used\n", cpu->curInstrPC, pc, pc &~ clrMask);

	cpu->regs[REG_NO_PC] = pc &~ clrMask;
}

static uint32_t cpuPrvGetRegNotPC(struct ArmCpu *cpu, uint_fast8_t reg)
{
	if (reg == REG_NO_PC)
		ERR("%s: you lied to me\n", __func__);
	return cpu->regs[reg];
}
...
{
	if (reg == REG_NO_PC)
		cpuPrvSetPCuninterworked(cpu, val);
	else
...
static bool cpuPrvSwitchToMode(struct ArmCpu *cpu, uint_fast8_t newMode)
{
	...
	if (saveTo == getFrom)
		return true;	//we're done if no regs to switch [this happens if we switch user<->system]
...
static bool cpuPrvSetPSRlo8(struct ArmCpu *cpu, uint_fast8_t val)
{
	if (!cpuPrvSwitchToMode(cpu, val & ARM_SR_M))
		return false;
...
bool cpuSetReg(struct ArmCpu *cpu, uint_fast8_t reg, uint32_t val)
{
	if (reg == ARM_REG_NUM_CPSR) {
		
		if (!cpuPrvSetPSRlo8(cpu, val))
			return false;
...
static void cpuPrvException(struct ArmCpu *cpu, uint32_t vector_pc, uint32_t lr, uint_fast8_t newLowBits)	//enters arm mode
{
	...
	if (!cpuPrvSetPSRlo8(cpu, newLowBits))
		ERR("cannot set PSR for exc");	//unlukely
	cpu->SPSR = spsr;
...
static uint32_t cpuPrvArmAdrMode_1(struct ArmCpu *cpu, uint32_t instr, bool* carryOutP, bool wasT, bool specialPC)
{
	...
	if (instr & 0x02000000UL) {				//immed
		...
		if (instr & 0x00000010UL) {			//reg with reg shift
			...
			a = cpuPrvGetRegNotPC(cpu, (instr >> 8) & 0x0F);	//get the relevant part of Rs, we only care for lower 8 bits (note we use uint8 for this)
...
static bool cpuPrvSetPSR(struct ArmCpu *cpu, uint_fast8_t mask, bool privileged, bool R, uint32_t val)
{
	if (R)	//setting SPSR in sys or usr mode is no harm since they arent used, so just do it without any checks
		cpu->SPSR = val;
	else {
		
		if (mask & 1) {		//set priviledged state
			
			if (privileged && !cpuPrvSetPSRlo8(cpu, val))
				return false;
			else if (!privileged) {

				if ((cpuPrvMaterializeCPSR(cpu) & 0xff) != val) {

					fprintf(stderr, "user mdoe setting unsettable bits in PSR ignored\n");
				}
			}
		}
...
static void cpuPrvExecInstr(struct ArmCpu *cpu, uint32_t instr, bool wasT , bool privileged, bool specialPC/* for thumb*/)
{
	...
	//pixter color color_vm debug
	if (0) {
		bool isMultimedia = true;

		const uint32_t pcVal = isMultimedia ? 0x0002B84E : 0x4004404C;
		const uint32_t vmMemLoc = isMultimedia ? 0x000771C4: 0x22A8;
		const uint32_t localsLocLoc = isMultimedia ? 0x000771C0 : 0x22A4;
		const uint32_t localsLocMaxSane = isMultimedia ? 0x00400000 : 0x4000;

		static uint32_t mSpInitial, mLocalsInitial;

		if (cpu->curInstrPC == pcVal) {
		
			uint32_t i, words[4] = {0,}, localsLoc = 0xffff0000;
			uint16_t mem[8], locals[8];

			for (i = 0; i < sizeof(words) / sizeof(*words); i++) {
				if (!cpuPrvMemOp(cpu, words + i, cpu->regs[13] + 4 * i, 4, false, true, NULL))
					fprintf(stderr, "  ** cant read W%u\n", i);
			}
			for (i = 0; i < sizeof(mem) / sizeof(*mem); i++) {
				if (!cpuPrvMemOp(cpu, mem + i, vmMemLoc + 2 * i, 2, false, true, NULL))
					fprintf(stderr, "  ** cant read M%u\n", i);
			}
			if (!cpuPrvMemOp(cpu, &localsLoc, localsLocLoc, 4, false, true, NULL))
				fprintf(stderr, "  ** cant read locals loc\n");
			for (i = 0; i < sizeof(locals) / sizeof(*locals); i++) {
				if (localsLoc > localsLocMaxSane || !cpuPrvMemOp(cpu, locals + i, localsLoc + 2 * i, 2, false, true, NULL)) {
					locals[i] = 0xf00f;
					fprintf(stderr, "  ** cant read L%u at 0x%08x\n", i, localsLoc + 2 * i);
				}
			}

			if (!mSpInitial)
				mSpInitial = cpu->regs[13];
			if (!mLocalsInitial)
				mLocalsInitial = localsLoc;

			fprintf(stderr, "vm[%08x]=%04x, SP(-%08x): {%04x %04x %04x %04x} MEM: %04x %04x %04x %04x %04x %04x %04x %04x LOC(+%08x): %04x %04x %04x %04x %04x %04x %04x %04x\n",
					cpu->regs[7], cpu->regs[0],
					mSpInitial - cpu->regs[13],
					words[0], words[1], words[2], words[3],
					mem[0], mem[1], mem[2], mem[3], mem[4], mem[5], mem[6], mem[7],
					localsLoc - mLocalsInitial,
					locals[0], locals[1], locals[2], locals[3], locals[4], locals[5], locals[6], locals[7]
				);
		}
	}

	//pixter color classic_VM
	if (0) if (cpu->curInstrPC == 0x400449EE) {
		
		uint32_t i, words[4] = {0,};
		uint8_t mem[8];
		...
		for (i = 0; i < sizeof(mem) / sizeof(*mem); i++) {
			if (!cpuPrvMemOp(cpu, mem + i, 0x22A8 + i, 1, false, true, NULL))
				fprintf(stderr, "  ** cant read M%u\n", i);
		}

		fprintf(stderr, "vm[%08x]=%02x, SP(%08x): {%02x %02x %02x %02x} MEM: %02x %02x %02x %02x %02x %02x %02x %02x\n",
				cpu->regs[7], cpu->regs[0], cpu->regs[13], words[0], words[1], words[2], words[3],
				mem[0], mem[1], mem[2], mem[3], mem[4], mem[5], mem[6], mem[7]);
	}
	...
	switch ((instr >> 24) & 0x0F) {
		...
		case 1:		//data processing immediate shift, register shift and misc instrs and mults
			if ((instr & 0x00000090UL) == 0x00000090) {		//multiplies, extra load/stores (table 3.2)
				...
				mul32:
							res += cpuPrvGetRegNotPC(cpu, (instr >> 8) & 0x0F) * cpuPrvGetRegNotPC(cpu, instr & 0x0F);
						...
						case 15:
							...
							res64  = cpuPrv64FromHalves(cpuPrvGetRegNotPC(cpu, (instr >> 16) & 0x0F), cpuPrvGetRegNotPC(cpu, (instr >> 12) & 0x0F));
							
				mul64:
							if (!cpu->haveLongMultiplies)
								goto invalid_instr;
					...
					if (mode & ARM_MODE_3_LOAD) {
						switch (mode & ARM_MODE_3_TYPE) {
							case ARM_MODE_3_H:
								...
								ok = cpuPrvMemOp(cpu, &memVal16, ldrEa, 2, false, privileged, &fsr);
								if (!ok) {
									cpuPrvHandleMemErr(cpu, ea, 2, false, false, fsr);
									if (cpu->baseUpdatedExcModel)
										goto instr_wbak_mode3;
									else
										goto instr_done;
								}
								memVal32 = memVal16;//needed since unaligned load16 will write top bits too...yes
								if (!cpu->doesAlignCheck && (ea & 1)) {

									uint_fast8_t rorBy = 8 * (ea & 1);

									memVal32 = (memVal32 >> rorBy) | (memVal32 << (32 - rorBy));
								}
								cpuPrvSetRegNotPC(cpu, (instr >> 12) & 0x0F, memVal32);
								break;
							...
							case ARM_MODE_3_SH:
								...
								if (!cpu->doesAlignCheck && (ea & 1)) {
									...
									memVal16 = (int16_t)(int8_t)(memVal16 >> 8);
								}
				...
				switch ((instr >> 4) & 0x0F) {
					...
					case 0:		//move reg to PSR or move PSR to reg
						...
						if ((instr & 0x00BF0FFFUL) == 0x000F0000UL) {		//move PSR to reg
							...
							if (!cpuPrvSetPSR(cpu, (instr >> 16) & 0x0F, privileged, !!(instr & 0x00400000UL), cpuPrvGetReg(cpu, instr & 0x0F, wasT, specialPC)))
								goto invalid_instr;
						}
						else
							goto invalid_instr;
					...
					case 3:
						...
						if (instr & 0x00400000UL) {		//CLZ
							...
							if ((instr & 0x00000030UL) == 0x00000030UL) {		//BLX?
							...
							cpuPrvSetPCinterworked(cpu, cpuPrvGetReg(cpu, instr & 0x0F, wasT, specialPC));
						}
						goto instr_done;
...
data_processing:							//data processing
				...
				switch ((instr >> 21) & 0x0F) {
					...
					case 9:			//TEQ
						if (!setFlags) {		//MSR CPSR, imm
							...
							if (!cpuPrvSetPSR(cpu, (instr >> 16) & 0x0F, privileged, false, cpuPrvROR(instr & 0xFF, ((instr >> 8) & 0x0F) * 2)))
								goto invalid_instr;
							goto instr_done;
						}
						op1 = cpuPrvGetReg(cpu, (instr >> 16) & 0x0F, wasT, specialPC);
					...
					case 11:		//CMN
						if (!setFlags) {		//MSR SPSR, imm
							...
							if (!cpuPrvSetPSR(cpu, (instr >> 16) & 0x0F, privileged, true, cpuPrvROR(instr & 0xFF, ((instr >> 8) & 0x0F) * 2)))
								goto invalid_instr;
							goto instr_done;
						}
						op1 = cpuPrvGetReg(cpu, (instr >> 16) & 0x0F, wasT, specialPC);
			...
			if (!setFlags)										//simple store
				...
				sr = cpu->SPSR;
				if (!cpuPrvSetPSRlo8(cpu, sr)) {

					fprintf(stderr, "SPSR copy to CPSR failed, SPSR was 0x%08x\n", sr);
					goto invalid_instr;
				}
				cpuPrvSetPSRhi8(cpu, sr);
				cpuPrvSetPCuninterworked(cpu, res);
			}
...
load_store_mode_2:
		...
		if (mode & ARM_MODE_2_LOAD) {
			uint_fast8_t dstReg = (instr >> 12) & 0x0F;
			...
			if (mode & ARM_MODE_2_WORD) {
				...
				ok = cpuPrvMemOp(cpu, &memVal32, ldrEa, 4, false, privileged, &fsr);
				if (!ok) {
					cpuPrvHandleMemErr(cpu, ea, 4, false, false, fsr);
					if (cpu->baseUpdatedExcModel)
						goto instr_wbak_mode2;
					else
						goto instr_done;
				}

				if (!cpu->doesAlignCheck && (ea & 3)) {

					uint_fast8_t rorBy = 8 * (ea & 3);

					memVal32 = (memVal32 >> rorBy) | (memVal32 << (32 - rorBy));
				}

				if (dstReg != REG_NO_PC)
					cpuPrvSetReg(cpu, dstReg, memVal32);
				else if (!cpu->haveV5plus)
					cpuPrvSetPCuninterworked(cpu, memVal32);
				else
					cpuPrvSetPCinterworked(cpu, memVal32);
			}
			else {
				...
				if (dstReg == REG_NO_PC && !cpu->haveV5plus) {

					if (memVal8 & 3)
						fprintf(stderr, "V5 corner case. at pc 0x%08x, 0x%08x is loaded to PC, whose lower 2 bits wil be ignored\n", cpu->curInstrPC, memVal8);

					memVal8 &=~ 3;
				}

				cpuPrvSetRegNotPC(cpu, dstReg, memVal8);
			}
		}
		else {
		...
		case 9:		//load/store multiple
			{
				...
				uint32_t loadedPc = 0xfffffffful, origBaseRegVal, origPC, origRegVal = 0;
				uint_fast8_t idx, regNo;
				
				origPC = cpu->regs[REG_NO_PC];
				...
				for (idx = 0; idx < 16; idx++) {
					...
					if (!ok) {
						cpu->regs[REG_NO_PC] = origPC;		//PC is restored!
						...
						if (cpu->baseUpdatedExcModel && (mode & ARM_MODE_4_WBK)) {	//update base if that abort model is used
							
							cpuPrvSetReg(cpu, mode & ARM_MODE_4_REG, origBaseRegVal + __builtin_popcount(regsList) * 4 * ((mode & ARM_MODE_4_INC) ? 1 : -1));
						}
						goto instr_done;
					}
					if (!(mode & ARM_MODE_4_BFR))
				...
				if (mode & ARM_MODE_4_WBK) {
					...
					origRegVal = cpuPrvGetRegNotPC(cpu, mode & ARM_MODE_4_REG);
				...
				if (copySPSR) {
					...
					if (!cpuPrvSetPSRlo8(cpu, sr)) {
						...
						if (mode & ARM_MODE_4_WBK)	//undo that writeback
							cpuPrvSetRegNotPC(cpu, mode & ARM_MODE_4_REG, origRegVal);

						fprintf(stderr, "SPSR copy to CPSR failed, SPSR was 0x%08x\n", sr);
						goto invalid_instr;
					}
					cpuPrvSetPSRhi8(cpu, sr);
					cpuPrvSetPCuninterworked(cpu, loadedPc);
				}
			}
			goto instr_done;
	...
	coproc_mem_2reg:
			...
			if (mode & ARM_MODE_5_RR) {		//handle MCRR, MRCC
				...
				if (specialInstr && !cpu->haveV5plus)		//LDC2/STC2
					goto invalid_instr;
	...
	coproc_dp:
			...
			if (instr & 0x00000010UL) {		//MCR[2]/MRC[2]
				...
				if (specialInstr && !cpu->haveV5plus)		//MCR2/MRC2
					goto invalid_instr;
				...
				if (specialInstr && !cpu->haveV5plus)		//CDP2
					goto invalid_instr;
...
static void cpuPrvCycleArm(struct ArmCpu *cpu)
{
	...
	priviledged = cpu->M != ARM_SR_MODE_USR;
	...
	ok = cpuPrvMemOpEx(cpu, &instr, fetchPc, 4, false, priviledged, &fsr, 0);
	if (!ok)
		cpuPrvHandleMemErr(cpu, pc, 4, false, true, fsr);
	else {
		cpu->regs[REG_NO_PC] += 4;
		cpuPrvExecInstr(cpu, instr, false, priviledged, false);
	}
}

static void cpuPrvCycleThumb(struct ArmCpu *cpu) {
	
	bool priviledged, vB, specialPC = false, ok;
	...
	if (!cpu->haveThumb) {
		...
		//as per docs, this causes an undefine dexception abort
		goto undefined;
	}
	...
	ok = cpuPrvMemOpEx(cpu, &instrT, fetchPc, 2, false, priviledged, &fsr, 0);
	if (!ok) {
			...
			if (instrT & 0x0800) {			// LDR(3)
				...
				switch ((instrT >> 8) & 3) {
					...
					case 3:			// BX
						...
						if (instrT == 0x4778) {	//special handing for thumb's "BX PC" (results differ on different CPUs)

							cpuPrvSetPCinterworked(cpu, cpu->regs[REG_NO_PC] + 2);
							goto instr_done;
						}
...
instr_execute:
	cpuPrvExecInstr(cpu, instr, true, priviledged, specialPC);
...
void cpuReset(struct ArmCpu *cpu, uint32_t pc)
{
	...
	cpuPrvSetPCuninterworked(cpu, pc);
	mmuReset(cpu->mmu);
}

struct ArmCpu* cpuInit(uint32_t pc, struct ArmMem *mem, uint32_t flags, int debugPort, uint32_t cpuid, uint32_t cacheId)
{
	struct ArmCpu *cpu = (struct ArmCpu*)calloc(1, sizeof(*cpu));
	
	if (!cpu)
		ERR("cannot alloc CPU");
	

	switch (flags & CPU_CORE_MASK) {
		case CPU_CORE_ARM_V3:
			cpu->haveLongMultiplies = !!(flags & CPU_CORE_FTR_M);
			cpu->haveMem64ops = false;
			cpu->haveV4plus = false;
			cpu->haveV5plus = false;
			cpu->haveExtraLoads = false;
			cpu->haveDspExts = false;
			cpu->haveBx = false;
			cpu->haveThumb = false;
			break;

		case CPU_CORE_ARM_V4:
			cpu->haveLongMultiplies = !(flags & CPU_CORE_FTR_M);
			cpu->haveMem64ops = false;
			cpu->haveV4plus = true;
			cpu->haveV5plus = false;
			cpu->haveExtraLoads = true;
			cpu->haveDspExts = false;
			cpu->haveBx = !!(flags & CPU_CORE_FTR_T);
			cpu->haveThumb = !!(flags & CPU_CORE_FTR_T);
			break;

		case CPU_CORE_ARM_V5:
			cpu->haveLongMultiplies = !(flags & CPU_CORE_FTR_M);
			cpu->haveMem64ops = (flags & CPU_CORE_FTR_T) && (flags & CPU_CORE_FTR_E) && !(flags & CPU_CORE_FTR_P);
			cpu->haveV4plus = true;
			cpu->haveV5plus = true;
			cpu->haveExtraLoads = true;
			cpu->haveDspExts = (flags & CPU_CORE_FTR_T) && (flags & CPU_CORE_FTR_E);
			cpu->haveBx = true;
			cpu->haveThumb = !!(flags & CPU_CORE_FTR_T);
			break;
	}
	cpu->baseUpdatedExcModel = !!(flags & CPU_CORE_FTR_BASE_UPDATED_EXC_MODEL);
	cpu->canConfigAlignCheck = !!(flags & CPU_CORE_FTR_CAN_CONFIG_ALIGN_CHECK);
	cpu->doesAlignCheck = !!(flags & CPU_CORE_FTR_DEFAULT_ALIGN_CHECK);
	cpu->hasPcBit1 = !!(flags & CPU_CORE_FTR_PC_BIT_1_IMPL);

	if (!(flags & CPU_CORE_MASK))
		ERR("cpu core not specified");

	if (!(flags & CPU_SOC_TYPE_MASK))
		ERR("cpu soc type not specified");

	if ((flags & CPU_CORE_FTR_M) && (flags & CPU_CORE_FTR_E))
		ERR("incompatible CPU flags: M & E");

	if (!(flags & CPU_CORE_FTR_T) && (flags & CPU_CORE_FTR_E))
		ERR("incompatible CPU flags: !T & E");

	if (!(flags & CPU_CORE_FTR_E) && (flags & CPU_CORE_FTR_P))
		ERR("incompatible CPU flags: !E & P");

	if ((flags & CPU_CORE_FTR_P) && (flags & CPU_CORE_FTR_M))
		ERR("incompatible CPU flags: P & M");

	if ((flags & CPU_CORE_FTR_T) && (flags & CPU_CORE_MASK) == CPU_CORE_ARM_V3)
		ERR("incompatible CPU flags: ARMv3 with T");

	if (!(flags & CPU_CORE_FTR_T) && (flags & CPU_CORE_MASK) == CPU_CORE_ARM_V5)
		ERR("incompatible CPU flags: ARMv5 without T");

	if ((flags & CPU_CORE_FTR_E) && (flags & CPU_CORE_MASK) != CPU_CORE_ARM_V5)
		ERR("incompatible CPU flags: E & archVer < 5");

	if ((flags & CPU_CORE_FTR_P) && (flags & CPU_CORE_MASK) != CPU_CORE_ARM_V5)
		ERR("incompatible CPU flags: P & archVer < 5");
	...
	cpu->mmu = mmuInit(mem, (flags & CPU_SOC_TYPE_MASK) == CPU_SOC_TYPE_XSCALE);
	if (!cpu->mmu)
		ERR("Cannot init MMU");

	cpu->cp15 = cp15Init(cpu, cpu->mmu, cpuid, cacheId, (flags & CPU_SOC_TYPE_MASK) == CPU_SOC_TYPE_XSCALE, (flags & CPU_SOC_TYPE_MASK) == CPU_SOC_TYPE_OMAP);
	if (!cpu->mmu)
...
void cpuAlignCheckingStateSet(struct ArmCpu *cpu, bool on)
{
	if (cpu->canConfigAlignCheck)
		cpu->doesAlignCheck = on;
}

bool cpuAlignCheckingStateGet(struct ArmCpu *cpu)
{
	return cpu->doesAlignCheck;
}
