...
struct stub {
	...
	static bool gdbStubPrvAddRegToStr(struct stub *stub, char *out, uint32_t regNo)
	{
		if (regNo < 15)			//normal regs
			...
			sprintf(out + strlen(out), "%08lx", (unsigned long)__builtin_bswap32(cpuGetRegExternal(stub->cpu, regNo) | ((cpuGetRegExternal(stub->cpu, ARM_REG_NUM_CPSR) & 0x20) ? 1 : 0)));
		else if (regNo == 16)		//CPSR
	...
	static bool gdbStubPrvInterpPacket(struct stub *stub)	//return true if we prepared a reply
	{
		...
		if (in == strstr(in, "qSupported"))
			strcpy(out, "PacketSize=1024;qXfer:features:read+");
	
		else if (in == strstr(in, "qXfer:features:read:target.xml:0,fff"))
			strcpy(out, "l<?xml version=\"1.0\"?><!DOCTYPE target SYSTEM \"gdb-target.dtd\"><target><architecture>armv4t</architecture><feature name=\"org.gnu.gdb.arm.core\"><reg name=\"r0\" bitsize=\"32\"/><reg name=\"r1\" bitsize=\"32\"/><reg name=\"r2\" bitsize=\"32\"/><reg name=\"r3\" bitsize=\"32\"/><reg name=\"r4\" bitsize=\"32\"/><reg name=\"r5\" bitsize=\"32\"/><reg name=\"r6\" bitsize=\"32\"/><reg name=\"r7\" bitsize=\"32\"/><reg name=\"r8\" bitsize=\"32\"/><reg name=\"r9\" bitsize=\"32\"/><reg name=\"r10\" bitsize=\"32\"/><reg name=\"r11\" bitsize=\"32\"/><reg name=\"r12\" bitsize=\"32\"/><reg name=\"sp\" bitsize=\"32\"/><reg name=\"lr\" bitsize=\"32\"/><reg name=\"pc\" bitsize=\"32\" type=\"code_ptr\"/><reg name=\"cpsr\" bitsize=\"32\"/></feature></target>");
			...
			out[0] = 0;
			for (regNo = 0; gdbStubPrvAddRegToStr(stub, out, regNo); regNo++);
		}
			...
			if (regNo < 15) {
				(void)cpuSetReg(stub->cpu, regNo, val);
				sprintf(out, "OK");
			}
			else if (regNo == 15) {	//cpuSetReg(PC, ...) does interworking. we need to be careful
				
				(void)cpuSetReg(stub->cpu, 15, (val &~ 1) | ((cpuGetRegExternal(stub->cpu, ARM_REG_NUM_CPSR) & ARM_SR_T) ? 1 : 0));
			}
			else if (regNo == 0x19) {
				(void)cpuSetReg(stub->cpu, ARM_REG_NUM_CPSR, val);
				sprintf(out, "OK");
			}
			else
				strcpy(out, "E05");
...
struct stub* gdbStubInit(struct ArmCpu *cpu, int port)
{
	...
	if (port < 0)
		stub->sock = -1;
			...
			(void)setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &(int){1}, sizeof(int));
