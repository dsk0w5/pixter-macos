...
bool disPrvGetU24(uint32_t addr, uint32_t *valP)
{
	uint8_t hi, mid, lo;

	if (!disPrvGetU8(addr, &lo) || !disPrvGetU8(addr + 1, &mid) || !disPrvGetU8(addr + 2, &hi)) {
		*valP = -1;
		return false;
	}

	*valP = (((uint_fast16_t)hi) << 16) + (((uint_fast16_t)mid) << 8) + lo;
	return true;
}
...
void disPrvSetType(uint32_t addr, uint_fast8_t type)		//does not set flags
{
	...
	if (addr >= sizeof(mFlags) / sizeof(*mFlags))
...
uint_fast8_t disPrvGetFlags(uint32_t addr, uint_fast8_t expectedType)
{
	...
	if (expectedType == FLAG_TYPE_UNEXPLORED)
		FATAL("gannot set flags of unexplored address 0x%04x\n", addr);
